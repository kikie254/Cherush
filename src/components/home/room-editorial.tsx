      ...
      <div className="flex flex-col justify-center space-y-8 lg:sticky lg:top-32 lg:self-start lg:py-20">
        ...
        >
          <div className="flex items-center gap-4 mb-4">
            ...
            <span className="text-sm tracking-[0.2em] uppercase text-accent font-medium">0{index + 1}</span>
          </div>
          <h3 className="font-display text-5xl md:text-6xl text-primary tracking-tight">{room.name}</h3>
          
          <div className="mt-6 flex flex-wrap gap-2">
            {room.features.slice(0, 4).map((feature, i) => (
              <span key={feature} className="px-4 py-1.5 rounded-full border border-primary/10 text-xs uppercase tracking-widest text-primary">
                {feature}
              </span>
            ))}
          </div>
          ...
          <div className="mt-10 flex items-center justify-between border-t border-primary/10 pt-8 max-w-lg">
            ...
            <div className="flex gap-4">
              <Button href={`/rooms/${room.slug}`} variant="secondary" className="px-6">Details</Button>
